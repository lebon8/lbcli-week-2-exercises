...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq -r '.vout | .[0] | .value' | awk '{s+=$1} END {print int(s*1000000 + 0.5)}')
...
TOTAL_AMOUNT=$(bitcoin-cli -regtest decoderawtransaction $BASE_TX | jq -r '.vout | .[0] | .value' | awk '{s=$1} END {print int(s*100000000 + 0.5)}' )
CHANGE_AMOUNT=$(( $TOTAL_AMOUNT - $PAYMENT_AMOUNT  ))
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RBF=$(bitcoin-cli -regtest decoderawtransaction $RAW_TX | jq -r '.vin | .[0] | .sequence' | awk 's=$1 ($s > 1 && $s < 4294967295) {print "true"}')
check_cmd "RBF verification" "VERIFY_RBF" "$VERIFY_RBF"

VERIFY_PAYMENT=$(bitcoin-cli -regtest decoderawtransaction $RAW_TX | jq -r '.vout | .[0] | .value')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(bitcoin-cli -regtest decoderawtransaction $RAW_TX | jq -r '.vout | .[1] | .value')
...
CHILD_TX_SIZE=$(bitcoin-cli -regtest decoderawtransaction $RAW_TX | jq -r ".vsize")
check_cmd "Child transaction size calculation" "CHILD_TX_SIZE" "$CHILD_TX_SIZE"

CHILD_FEE_RATE=20 # satoshis/vbyte
CHILD_FEE_SATS=$(awk -v fee=$CHILD_FEE_RATE -v size=$CHILD_TX_SIZE 'BEGIN {print fee * size }')
...
CHILD_SEND_AMOUNT=$(bitcoin-cli -regtest decoderawtransaction $RAW_TX | jq -r ".vout | .[0] | .value" | awk '{s=$1} END {print int(s*100000000 + 0.5)}')
...
# STUDENT TASK: Create the outputs JSON structure
CHILD_OUTPUTS=S='[{"'$CHILD_RECIPIENT'":"'$CHILD_SEND_BTC'"}]'
...
SECONDARY_TXID=D=$(bitcoin-cli -regtest decoderawtransaction $SECONDARY_TX | jq -r ".txid")
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_INPUTS=S='[{"txid":"'$SECONDARY_TXID'","vout":0,"sequence":"0000000a"}]'
...
SECONDARY_OUTPUT_VALUE=E=$(bitcoin-cli -regtest decoderawtransaction $SECONDARY_TX | jq -r '.vout | .[1] | .value' | awk '{s=$1} END {print int(s*100000000 +0.5)}')
...
TIMELOCK_AMOUNT=T=$(awk -v fee=$TIMELOCK_FEE -v value=$SECONDARY_OUTPUT_VALUE 'BEGIN {print value - fee}')
check_cmd "Timelock amount calculation" "TIMELOCK_AMOUNT" "$TIMELOCK_AMOUNT"

# Convert to BTC
TIMELOCK_BTC=C=$(printf "%.8f" $(echo "scale=8; $TIMELOCK_AMOUNT / 100000000" | bc -l))

# STUDENT TASK: Create the outputs JSON structure
TIMELOCK_OUTPUTS=S='[{"'$TIMELOCK_ADDRESS'":'$TIMELOCK_BTC'}]'
...
TIMELOCK_TX=X=$(bitcoin-cli -regtest -named createrawtransaction inputs=$TIMELOCK_INPUTS outputs=$TIMELOCK_OUTPUTS)
